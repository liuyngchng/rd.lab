...
## 工作原理
  ...
  RAG 的工作流程 可以简单地分为以下四个阶段。

（1）数据处理。收集和预处理相关数据，以确保信息的质量和可用性。将非结构化的离散的文本处理成结构化数据（一下简称`数据1`），以供大模型使用。

（2）检索阶段。从知识库中检索与用户查询相关的信息，确保获取最新和最相关的数据。当用户输入内容（`数据0`）时，首先在`数据1`中进行相关检索，输出`数据2`。

（3）增强阶段。将检索到的信息（`数据2`）与用户输入内容（`数据0`）结合，为模型提供丰富的上下文（`数据3`）。

（4）生成阶段。基于增强的信息，使用大型语言模型生成最终的回答或内容。即将`数据3` 输入大模型，由大模型输出 `数据4`。

 以 LangChain为例，其工作流程示意图如图 11-1 所示。

<img src="img/rag_principles.png" style="zoom:40%;" />

<center><b>图 11-1  LangChain 工作流程示意图</b><center>
