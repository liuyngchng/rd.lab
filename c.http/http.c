...
char *const getpl(const char *ip, const int port, const char *method, const char *path,
		const char *body, char *const pl, int n) {
	char header[512] = { 0 };
	char debug[1024] = { 0 };
	sprintf(debug,
	...
	if (strncmp(method, "GET", 3) == 0) {
		sprintf(pl, "%sContent-Length:0\r\n\r\n", header);
	} else {
		sprintf(pl, "%sContent-Length:%ld\r\n\r\n%s", header, strlen(body),
			body);
	}
...
char *const req(const char *ip, const int port, const char *method,
    ...
    char pl[1024] = {0};
	getpl(ip, port, method, path, body, pl, sizeof(pl));
    char raw_resp[4096] = {0};
    writemsg(ip, port, pl, raw_resp);
