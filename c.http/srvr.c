...
int startsrv() {
    ...
    printf("[%s][%s-%d][t-%ld]listening %d\n",gettime(), filename(__FILE__),__LINE__,pthread_self(),_SRV_PORT_);
    while (1) {
...
again:  cfd = accept(sfd, (struct sockaddr *)&cliaddr, &len);
        ...
        printf("[%s][%s-%d][t-%ld]%s connected\n", gettime(), filename(__FILE__), __LINE__, pthread_self(), ip);
        pthread_t t;
...
void *rcvdata(void* sockfd) {
    ...
    printf("[%s][%s-%d][t-%ld]rcv %d bytes\n%s\n",gettime(), filename(__FILE__),__LINE__, pthread_self(), size, buf);
    char resp[4096] = {0};
    dispatch(buf,resp);
    write(cfd, resp, strlen(resp));
    printf("[%s][%s-%d][t-%ld]return msg\n%s\n", gettime(), filename(__FILE__), __LINE__, pthread_self(),resp);
    close(cfd);
