...
十二平均律将一个纯八度音程（频率比为 2:1，即转一圈后声音频率升高1倍）**平均等分**为12份。

- 每个等分称为一个“半音”（小二度）。

- 相邻两个键（无论是白键还是黑键）之间的频率比是固定的：
  $$
  \sqrt[12]{2} \approx 1.05946
  $$

- 这意味着，每升高一个半音，声波的频率乘以 1.05946；每降低一个半音，频率除以 1.05946。
...
和弦的“色彩”背后，是精确的数学和物理关系，十二平均律，每相邻的2个音之间为一个半音。

在十二平均律下，音程由**相隔的半音数量**来精确定义：

- **纯五度 (Perfect 5th)**：7个半音（例如 C → G）

- **大三度 (Major 3rd)**：4个半音（例如 C → E）

- **小三度 (Minor 3rd)**：3个半音（例如 C → E♭）

虽然十二平均律是“平均”的，但它非常接近自然中存在的简单整数比频率关系，这种简单比例决定了协和度与色彩。

大三和弦 (C-E-G) 的近似频率比 (以C=1为基准)：

- C(根音)： 1
- E(大三度)： ≈ 1.2599 (在纯律中是 5/4 = 1.25)
- G(纯五度)： ≈ 1.4983 (在纯律中是 3/2 = 1.5)

这个和弦内部的频率关系（如E与C的5:4， G与C的3:2， G与E的约6:5）构成了简单、明亮的整数比组合，因此听起来协和、稳定、开朗。

小三和弦 (C-E♭-G) 的近似频率比 (以C=1为基准)：

- C(根音)： 1
- E♭(小三度)： ≈ 1.1892 (在纯律中是 6/5 = 1.2)
- G(纯五度)： ≈ 1.4983 (在纯律中是 3/2 = 1.5)

虽然它同样协和（包含纯五度3:2），但其核心音程——小三度（C与E♭的 6:5 比例）比大三度的 5:4 比例略复杂一点。这个“略复杂”的共鸣产生了内敛、柔和、略带忧郁或沉思的色彩感。
...
音乐记谱软件，支持吉他指法标注, 还具有以下特点

- 乐谱和吉他指法同步显示
- 可插入指法图到乐谱中
- 支持LilyPond导出（高质量印刷效果）
