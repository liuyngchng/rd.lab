...
（1）进入路由器管理界面（http://tplogin.cn, http://192.168.1.253）, 设置DHCP服务为启用（默认为自动），修改地址池持号段如下

```sh
地址池开始地址: 172.20.10.3   # 1为手机， 2为路由器
地址池结束地址: 172.20.10.14
网关:			172.20.10.1		# 手机热点IP, 可以通过其他电脑无线网卡直接连接手机，看看电脑上的网关和DNS是啥
首选DNS服务器: 172.20.10.1		# 手机热点IP
```
...
（3）电脑网线连接路由器，若可以直接连接172.20.10.2, 则跳过这一步，进入下一步。

若无法连接，则执行本步骤。由于电脑自动分配的网段为192.168.1.\*, 无法直接与路由器直接相连。需要设置为手动分配IP
